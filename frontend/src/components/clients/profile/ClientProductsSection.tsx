...
const REL_VALUES: ClientProductRelationship[] = [
  "preferred",
  "frequent",
  "recent",
  "interested",
...
export default function ClientProductsSection({
  ...
  const relOptions = REL_VALUES.map((value) => ({
    value,
    label: productAffinityLabel(value),
  }));
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-zinc-900">
            {t("clients.preferredProductsTitle")}
          </h2>
          <p className="mt-0.5 text-xs text-zinc-500">
            {t("clients.productsSectionHint")}
            <a href="/products" className="font-medium text-[var(--vx-accent)] hover:underline">
              {t("clients.productsCatalogLink")}
            </a>
          </p>
        </div>
        <button
        ...
        >
          {adding ? t("clients.closeLink") : t("clients.linkProductBtn")}
        </button>
      </div>

      {links.length === 0 ? (
        <p className="mt-3 text-sm text-zinc-500">
          {t("clients.noProductsLinked")}
        </p>
      ) : (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-zinc-500">
                  {productAffinityLabel(link.relationship)}
              ...
              <button
                ...
                aria-label={t("clients.removeProductAria")}
              >
                ×
              </button>
            </li>
        ...
        <div className="mt-4 space-y-3 rounded-xl border border-zinc-100 bg-zinc-50/40 p-3">
          ...
          {available.length > 0 ? (
            <select
              ...
              className="w-full rounded-lg border border-zinc-200 px-2 py-1.5 text-sm"
            >
              <option value="">{t("clients.selectCatalog")}</option>
              {available.map((p) => (
          ...
          <p className="text-[11px] text-zinc-500">{t("clients.orAddCatalog")}</p>
          <input
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            placeholder={t("clients.productName")}
